...
The *associate_phone_with_user.py* script creates a new phone association with user *testuser*. In the script JSON output find the *user_id* value and copy it for use in the next script.

Open the *get_phone_by_userid.py* file and paste the *user_id* value into the `USER_ID = ''` variable.  Save the file.
